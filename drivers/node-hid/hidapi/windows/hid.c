...
int HID_API_EXPORT HID_API_CALL hid_write(hid_device *dev, const unsigned char *data, size_t length)
{
	...
	res = WriteFile(dev->device_handle, buf, length, NULL, &dev->write_ol); // write time-out
...
read_entrance:
	...
	if (res && bytes_read > 0) {
		...
		bytes_read = dev->read_buf[1];
		if(bytes_read==0) goto read_entrance;
		memcpy(data, dev->read_buf+2, bytes_read);
#endif
	}
