...
{
    // yzj, modified for MeHID
    send_buf[0] = 0; // report id
    send_buf[1] = length;
    //printf("send %d bytes!!\n",length);
    memcpy(send_buf+2,data,length);
    return set_report(dev, kIOHIDReportTypeOutput, send_buf, length+2);
...
read_entry:
	...
	if (dev->input_reports) {
        ...
        // yzj, don't return for 0 len report
        if(bytes_read==0) goto read_entry;
		goto ret;
	}
